...
export default function StateOfNWCard({ title, image, date, time, url }: { title: string, image: StaticImageData; date: string; time: string, url: string }) {
  return (
    <Link
      ...
      target="_blank"
      href={url}
      className="flex flex-col p-4 border-r border-dashed shrink-0 group relative hover:bg-fd-muted/70 transition-colors duration-300">
      <div className="relative mb-3 bg-black rounded-2xl border">
        <div className="absolute top-1/2 left-1/2 -translate-1/2 rounded-full bg-gradient-to-br from-fd-background to-fd-accent p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 overflow-clip">
          {/* shimmer */}
          <div className="w-2 h-full rotate-45 bg-none dark:bg-white absolute -top-7 -left-2 delay-150 group-hover:duration-1000 group-hover:translate-16 blur-sm" />
          <Play className="fill-current translate-x-0.5 size-6 stroke-none text-fd-primary drop-shadow-2xl drop-shadow-fd-primary" />
        </div>
