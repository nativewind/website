...
export default function CommunityPost({
  ...
  <div className="flex flex-col border-r border-dashed shrink-0 relative">
    <div className="group/card flex m-6 sm:m-2 rounded-2xl max-sm:shadow-xl hover:shadow-xl flex-col-reverse max-sm:bg-white max-sm:dark:bg-fd-muted hover:bg-white dark:hover:bg-fd-muted duration-300">
      <Link
        target="_blank"
        href={url}
        className="flex flex-col m-2 mt-0 rounded-lg relative group group-hover/card:bg-fd-background duration-300 border group-hover/card:border-fd-border border-transparent max-sm:border-fd-border max-sm:bg-fd-background">
        ...
        {/* </div> */}
        {image ? 
        <Image src={image} alt={`${title} Thumbnail`} width={1280} height={720} className="aspect-video object-cover rounded-t-lg h-auto w-76 mb-4" />
        : null}
        <h2 className="text-lg font-semibold flex gap-2 group-hover/card:translate-x-4 max-sm:translate-x-4 duration-300">
          {title}
        ...
        <p className="text-fd-muted-foreground text-sm max-w-68 mb-4 line-clamp-2 group-hover/card:translate-x-4 max-sm:translate-x-4 duration-300 group-hover/card:delay-50">
          {description}
        </p>
      </Link>
      <div className="p-3 flex gap-8 justify-between items-center">
        ...
        ].filter(i => i && i.href && i.href.length > 0)} />
        </div>
      </div>
    </div>
  )
