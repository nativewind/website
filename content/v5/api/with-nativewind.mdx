...
To keep a CSS variable dynamic instead of compiling its default value into native styles:

```js
module.exports = withNativewind(config, {
  inlineVariables: { exclude: ["--color-primary"] },
});
```

See [Dynamic Themes](/v5/guides/themes) for provider examples.
