...
`vars()` is deprecated but remains available in RC0. Prefer `VariableContextProvider` for new code. Preserve runtime variables with `inlineVariables.exclude` when the compiler would otherwise inline them; see [Dynamic Themes](/v5/guides/themes).
...
function ThemedSection({ brandColor }) {
  return (
    <View style={vars({ "--brand-color": brandColor })}>
      <Text className="text-[var(--brand-color)]">Themed text</Text>
    </View>
  );
}
```
...
function App() {
  return (
    <VariableContextProvider value={{ "--theme-color": "#3b82f6" }}>
      <Text className="text-[var(--theme-color)]">Blue text</Text>
    </VariableContextProvider>
  );
}
```
