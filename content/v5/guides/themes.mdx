...
Preserve each variable you override at runtime from native compiler inlining:
...
module.exports = withNativewind(getDefaultConfig(__dirname), {
  inlineVariables: {
    exclude: ["--color-primary", "--color-secondary"],
  },
});
```

Add other dynamic tokens to this list, including `--color-foreground` and `--color-muted-foreground` in the multiple themes example below. Use units for cross platform length variables, such as `"80px"`.
...
`vars()` remains available but deprecated in RC0. For new code or a planned refactor, use `VariableContextProvider`:
