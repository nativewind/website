...
The simplest way to migrate from Nativewind v4 -> Nativewindv5 is to follow the [installation steps](/docs/getting-started/installation) again.
...
You should now be running Nativewind v5 🎉 If you are running into issues, here are some common steps that may have been overlooked:
...
{/* TODO: Add monorepo migration steps */}

**Metro config**

The `withNativewind` function no longer requires a second argument. Please note that, while `withNativeWind` was renamed to `withNativewind`, either will work.
...
Ensure that the `postcss.config.mjs` file was created!

**JSX transform errors**

Ensure that you removed `nativewind` from your babel config. You will most likely need to clear the Metro cache by running `npx expo start --clear`.
