...
The simplest way To migrate from v4 -> v5 is to follow the [installation steps](/docs/getting-started/installation) again.

Once completed, the only additional step is to remove `nativewind` from your `babel.config.js`
...
You should now be running v5 🎉 If you are running into issues, are some common steps that may have been overlooked

**Incorrect CSS atRules**

Ensure you updated to the new atRules in your CSS file
...
The `withNativewind` no longer requires a second argument. `withNativeWind` was renamed to `withNativewind`, but both will work.
...
Ensure that this file was created!

**JSX transform errors**

Ensure you removed `nativewind` from your babel config. You will most likely need to clear the Metro cache.
